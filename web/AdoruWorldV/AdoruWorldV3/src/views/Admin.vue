    ...
    <div class="bg-sky border-b-4 border-pokemon-black px-4 py-3">
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-4">
          <router-link to="/" class="pixel-text text-base text-pokemon-black hover:text-gold-dark">← 返回前台</router-link>
          <h1 class="pixel-text text-base text-pokemon-black">⚙️ 管理后台</h1>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
      <div class="flex gap-2 mb-6 flex-wrap">
        <button
          ...
          class="pixel-btn text-sm py-2 px-4"
      ...
      <div class="flex justify-between items-center mb-4">
        <h2 class="pixel-text text-base text-pokemon-black">
          {{ tabs.find(t => t.id === activeTab)?.name }}列表
        </h2>
        <button @click="openCreate" class="pixel-btn bg-gold text-pokemon-black text-sm py-2 px-4">
          + 新增
        </button>
      </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gold-light border-b-4 border-pokemon-black">
              <th class="pixel-text text-sm text-left p-3">名称</th>
              <th v-if="activeTab === 'posts'" class="pixel-text text-sm text-left p-3 hidden md:table-cell">状态</th>
              <th v-if="activeTab === 'posts'" class="pixel-text text-sm text-left p-3 hidden md:table-cell">浏览</th>
              <th v-if="activeTab === 'dex'" class="pixel-text text-sm text-left p-3 hidden md:table-cell">分类</th>
              <th v-if="activeTab === 'dex'" class="pixel-text text-sm text-left p-3 hidden md:table-cell">评分</th>
              <th v-if="activeTab === 'projects'" class="pixel-text text-sm text-left p-3 hidden md:table-cell">状态</th>
              <th class="pixel-text text-sm text-right p-3">操作</th>
            </tr>
          </thead>
          <tbody>
    ...
    <div v-if="showEditor" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showEditor = false">
      <div class="pixel-card p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="space-y-4">
          <div v-if="activeTab === 'posts'">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">标题</label>
                <input v-model="editingItem.title" @blur="generateSlug" class="pixel-input w-full text-sm" />
              </div>
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">Slug</label>
                <input v-model="editingItem.slug" class="pixel-input w-full text-sm" />
              </div>
            </div>
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">描述</label>
              <input v-model="editingItem.description" class="pixel-input w-full text-sm" />
            </div>
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">封面图</label>
              <div class="flex gap-2">
                <input v-model="editingItem.coverImage" class="pixel-input flex-1 text-sm" placeholder="图片URL或点击上传" />
                <label class="pixel-btn bg-sky-dark text-white text-sm py-2 px-4 cursor-pointer" :class="{ 'opacity-50': uploading }">
              {{ uploading ? '上传中...' : '📁 上传' }}
            ...
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">内容 (Markdown)</label>
              <textarea v-model="editingItem.content" class="pixel-input w-full text-sm" rows="12"></textarea>
            </div>
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">分类 (逗号分隔)</label>
              <input v-model="editingItem.categoryNames" class="pixel-input w-full text-sm" placeholder="技术笔记,项目记录" />
            </div>
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">标签 (逗号分隔)</label>
          ...
          <div v-if="activeTab === 'categories'">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">名称</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">图标</label>
                <input v-model="editingItem.icon" class="pixel-input w-full text-sm" placeholder="💻" />
              </div>
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">颜色</label>
          ...
          <div v-if="activeTab === 'projects'">
            ...
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">技术栈 (逗号分隔)</label>
              <input v-model="editingItem.tech" class="pixel-input w-full text-sm" placeholder="Vue 3,TypeScript" />
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">链接</label>
                <input v-model="editingItem.link" class="pixel-input w-full text-sm" />
              </div>
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">GitHub</label>
            ...
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">状态</label>
          ...
          <div v-if="activeTab === 'dex'">
            ...
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">原标题</label>
              <input v-model="editingItem.originalTitle" class="pixel-input w-full text-sm" />
            </div>
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">封面</label>
              <div class="flex gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">分类</label>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">评分</label>
                <input v-model.number="editingItem.rating" type="number" min="0" max="10" class="pixel-input w-full text-sm" />
              </div>
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">进度</label>
                <input v-model.number="editingItem.progress" type="number" class="pixel-input w-full text-sm" />
              </div>
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">总数</label>
                <input v-model.number="editingItem.total" type="number" class="pixel-input w-full text-sm" />
              </div>
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">年份</label>
                <input v-model.number="editingItem.year" type="number" class="pixel-input w-full text-sm" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">创作者</label>
                <input v-model="editingItem.creator" class="pixel-input w-full text-sm" />
              </div>
              <div>
                <label class="pixel-text text-sm text-pokemon-black block mb-1">平台</label>
            ...
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">类型 (逗号分隔)</label>
              <input v-model="editingItem.genre" class="pixel-input w-full text-sm" />
            </div>
            <div>
              <label class="pixel-text text-sm text-pokemon-black block mb-1">短评</label>
        ...
        <div class="flex justify-end gap-3 mt-6 pt-4 border-t-4 border-pokemon-black">
          <button @click="showEditor = false" class="pixel-btn bg-pokemon-gray text-pokemon-black text-sm py-2 px-4">取消</button>
          <button @click="saveItem" class="pixel-btn bg-gold text-pokemon-black text-sm py-2 px-4" :disabled="loading">
            {{ loading ? '保存中...' : '保存' }}
          </button>
        </div>
      </div>
