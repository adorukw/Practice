...
import type { TrajectoryEdge, TrajectoryNodeType } from '../types/trajectory'
import type { ParsedTrajectory } from '../types/trajectory'
import { parseTrajectoryEvents } from '../parser/trajectory'
import { summarizeContent, type SummarizeRequest } from '../services/summarizer'
import config from '../config.json'

const STORAGE_KEY_PREFIX = 'openclaw-traj-summary-'
...
export const useTrajectoryStore = defineStore('trajectory', () => {
  const nodes = ref<any[]>([])
  const edges = ref<TrajectoryEdge[]>([])
  const selectedNodeId = ref<string | null>(null)
  const trajectoryMeta = ref<ParsedTrajectory['trajectoryInfo'] | null>(null)
  const metadata = ref<any>(null)
  const filterType = ref<TrajectoryNodeType | 'all'>('all')
  ...
  function loadTrajectory(events: any[], meta?: any) {
    const parsed = parseTrajectoryEvents(events)
    nodes.value = parsed.nodes
    edges.value = parsed.edges
    trajectoryMeta.value = parsed.trajectoryInfo
    metadata.value = meta || null
    selectedNodeId.value = null

    const cached = loadCachedSummaries(parsed.trajectoryInfo.sessionId)
  ...
  function setFilterType(type: TrajectoryNodeType | 'all') {
  ...
  function persistSummaries() {
    if (trajectoryMeta.value) {
      saveCachedSummaries(trajectoryMeta.value.sessionId, summaries.value)
    }
  }
  ...
  async function runSummarize() {
    ...
    for (const node of nodes.value) {
      ...
      if (data.type === 'thinking') {
        ...
        if (!thinking) thinking = data.label
        requests.push({
          nodeId: node.id,
          req: {
            toolName: 'thinking',
        ...
        if (raw?.data?.arguments) {
          argsStr = JSON.stringify(raw.data.arguments)
        } else if (raw?.data?.message?.content) {
          const content = raw.data.message.content
          const toolCallBlocks = content.filter((b: any) => b.type === 'toolCall')
          argsStr = toolCallBlocks.map((b: any) => JSON.stringify(b.arguments || {})).join('\n')
        }
        requests.push({
          nodeId: node.id,
          req: {
