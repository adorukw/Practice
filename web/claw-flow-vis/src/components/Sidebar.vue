...
const filterOptions: { label: string; value: SessionNodeType | 'all' }[] = [
  { label: '全部', value: 'all' },
  { label: '用户', value: 'user' },
...
const typeTagMap: Record<string, { text: string; cls: string }> = {
  ...
  toolCall: { text: '调', cls: 'bg-orange-100 text-orange-700' },
  toolResult: { text: '果', cls: 'bg-green-100 text-green-700' },
}
  ...
  >
    ...
    <template v-if="!collapsed">
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">时间线</h3>
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              ...
              <div class="text-xs text-gray-400 mt-0.5">
                ...
                <span v-if="node.data?.meta?.toolName" class="ml-1.5 text-orange-500 font-medium">{{ node.data.meta.toolName }}</span>
              </div>
            </div>
          </button>
        </div>
