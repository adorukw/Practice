...
const resultText = computed(() => {
  const content = props.data.raw?.content
...
<template>
  <div
    class="toolresult-node px-4 py-3 rounded-lg border-2 transition-all duration-200"
    :class="{
      'bg-toolresult-bg border-toolresult-border shadow-sm': !isError && !isSelected,
      'bg-toolresult-bg border-toolresult ring-2 ring-toolresult/50 shadow-md': !isError && isSelected,
      'bg-error-bg border-error-border shadow-sm': isError && !isSelected,
      'bg-error-bg border-error ring-2 ring-error/50 shadow-md': isError && isSelected,
    }"
    style="width: 300px"
    @click="store.selectNode(id)"
  >
    <Handle type="target" :position="Position.Top" class="!bg-toolresult !w-2 !h-2" />
    <div class="flex items-center gap-2 mb-2">
      <span
        class="w-6 h-6 flex items-center justify-center rounded text-white text-[10px] font-bold"
        :class="isError ? 'bg-error' : 'bg-toolresult'"
      >
        {{ isError ? '错误' : '结果' }}
      </span>
      <span
        class="text-xs font-semibold tracking-wider"
        :class="isError ? 'text-error' : 'text-toolresult'"
      >
        {{ isError ? '执行错误' : '执行结果' }}
      </span>
    ...
    <div v-if="aiSummary" class="text-sm text-gray-700 leading-snug mb-1.5 font-medium border-l-2 pl-2" :class="isError ? 'border-error' : 'border-toolresult'">
      {{ aiSummary }}
    <div v-if="aiSummary" class="text-sm text-gray-700 leading-snug mb-1.5 font-medium">
      ✨ {{ aiSummary }}
    </div>
    ...
    <Handle type="source" :position="Position.Bottom" class="!bg-toolresult !w-2 !h-2" />
  </div>
  </div>
</template>
